...
final class Store: ObservableObject {
    ...
    private let fileName = "backorder_entries.json"
    ...
    static func seedData() -> [Entry] {
        let now = Date()
        return [
            Entry(title: "Sample Backorder One", detail: "First example entry.", date: now),
            Entry(title: "Sample Backorder Two", detail: "Second example entry.", date: now.addingTimeInterval(-86400)),
            Entry(title: "Sample Backorder Three", detail: "Third example entry.", date: now.addingTimeInterval(-172800)),
        ]
    }
}
